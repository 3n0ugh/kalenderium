<template>
  <v-row class="fill-height">
    <div v-if="loggingOut" class="pacman">
      <img src="../assets/pacman_loading.svg" alt="" />
    </div>
    <div v-if="errorMessage" class="alert alert-danger" role="alert">
      {{ errorMessage }}
    </div>
    <v-col v-if="!loggingOut">
      <v-sheet height="64">
  ...
  methods: {
    ...
    async addEvent() {
      ...
      if (this.name && this.start && this.end) {
        ...
        this.errorMessage = "You must enter event name, start, and end time";
      }
    },
    async deleteEvent(ev) {
      await axios.delete(`/v1/calendar/${ev}`, {
        headers: {
          Authorization: `Bearer ${localStorage.token}`,
        },
      });
    ...
    logout() {
      axios.post("/v1/logout", {
        token: {
          plaintext: localStorage.getItem("token"),
        },
      });
